...
def _month_bar_chart(month_df, title, height=400):
    fig = px.bar(
        month_df.with_columns(
            pl.col("month")
            .replace_strict(pl.Series(range(1, 13)), pl.Series(_MONTH_NAMES))
            .alias("month_name")
        ),
        x="month_name",
        y="count",
...
def get_license_counts(_lf):
    return (
        ...
        .sort("count", descending=True)
        .with_columns(
            pl.when(pl.col("license").is_null())
            .then(pl.lit("Missing"))
            .when(pl.col("license").str.contains("nonexclusive", literal=True))
...
def _dashboard_kpis(_lf):
    row = _lf.select(
        pl.len().alias("total"),
        pl.col("categories").str.split(" ").explode().unique().count().alias("n_cats"),
...
def search_papers(_lf, query, category, year_range, author):
    ...
    if query:
        ...
        cond = None
        for p in parts:
            c = pl.col("title").str.contains(p, literal=True) | pl.col(
                "abstract"
            ).str.contains(p, literal=True)
...
def page_dashboard():
    ...
    st.subheader(
        "Data Completeness",
        help="Proportion of non-null values for each column in the dataset",
    )
...
def page_search():
    ...
    with st.sidebar:
        ...
        author = st.text_input(
            "Author", placeholder="e.g. de Leeuw", key="search_author"
        )
            ...
            for row in page_df.iter_rows(named=True):
                with st.expander(f"{row['id']} — {row['title'][:120]}"):
                    ...
                    with cols[1]:
                        ...
                        n_auth = (
                            len(row["authors_parsed"]) if row["authors_parsed"] else 0
                        )
...
def _cat_counts_histogram(_lf):
    return (
        _lf.select(pl.col("categories").str.split(" ").list.len().alias("n_cats"))
        .collect()["n_cats"]
        .to_list()
    )
...
def _cooccurrence_matrix(_lf, top_categories):
    ...
    cooc_norm = cooc / totals[:, np.newaxis]
...
def page_categories():
    ...
    fig = px.histogram(
        x=cat_vals,
        nbins=10,
        title="Categories per Paper",
        labels={"x": "Categories", "count": "Papers"},
        height=400,
    )
    ...
    fig = px.bar(
        ...
        y="label",
        orientation="h",
        title=f"Top {top_n} Research Areas",
        labels={"count": "Papers", "label": ""},
        height=min(800, 200 + 18 * top_n),
        text_auto=True,
    )
    fig.update_traces(marker_color="#00cc96")
...
def page_authors():
    ...
    override = st.session_state.get("author_search_override", "")
    if override and search_name and search_name.strip():
        if search_name not in override:
            st.session_state.pop("author_search_override", None)
            override = ""
    effective = override or search_name

    if effective:
        MAX_SHOW = 200
        papers = (
            lf.filter(pl.col("authors").str.contains(effective, literal=True))
            .head(MAX_SHOW + 1)
            .collect()
        )
        ...
        if len(papers) > 0:
            ...
            if len(papers) > MAX_SHOW:
                st.warning(
                    f"Showing first {MAX_SHOW} of {total} papers. Refine your search."
                )
        ...
        fig = px.bar(
            ...
            y="authors_parsed",
            orientation="h",
            title="Top 30 Author Last Names",
    ...
    with st.spinner("Computing author distribution…"):
        st.subheader(
            "Authors per Paper Distribution", help=labels.COLUMN_HELP["n_authors"]
        )
...
def _comments_stats(_lf):
    return _lf.select(
        ...
        pl.col("comments")
        .str.contains(r"\d+\s*pages?", literal=False)
        .alias("has_pages"),
        pl.col("comments")
        .str.contains(r"\d+\s*figures?", literal=False)
        .alias("has_figures"),
        pl.col("comments")
        .str.contains(r"\d+\s*\w*\s*references?", literal=False)
        .alias("has_refs"),
    ).collect()
...
def page_trends():
        ...
        with st.spinner("Computing version statistics…"):
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "n_versions": st.column_config.NumberColumn(
                        "Versions", help=labels.COLUMN_HELP["n_versions"]
                    ),
                },
            )

            version_binned = (
                version_stats.with_columns(
                    pl.when(pl.col("n_versions") > 20)
                    .then(21)
                    .otherwise(pl.col("n_versions"))
                    .alias("n_vers_binned")
                )
    ...
    with tab3:
        with st.spinner("Analyzing text length…"):
            ...
            fig.add_trace(
                go.Histogram(
                    x=text_df["abstract_len"].clip(upper_bound=3000),
                    nbinsx=80,
                    marker_color="#00cc96",
                    name="Abstract",
                ),
            ...
            fig.update_layout(
                title_text="Text Length Distributions", height=450, showlegend=False
            )
            st.plotly_chart(fig, width="stretch")
            ...
            fig = px.scatter(
                ...
                labels={
                    "title_len": "Title (chars)",
                    "abstract_len": "Abstract (chars)",
                },
                opacity=0.3,
                height=500,
            )
    ...
    with tab4:
        with st.spinner("Parsing comments field…"):
            ...
            st.subheader(
                "Fields detected in comments",
                help="Counts of papers whose comments field mentions page counts, figure counts, or reference counts",
            )
            c1, c2, c3 = st.columns(3)
            c1.metric(
                "With page count",
                f"{comments_df['has_pages'].sum():,}",
                help=labels.COLUMN_HELP["has_pages"],
            )
            c2.metric(
                "With figure count",
                f"{comments_df['has_figures'].sum():,}",
                help=labels.COLUMN_HELP["has_figures"],
            )
            c3.metric(
                "With ref count",
                f"{comments_df['has_refs'].sum():,}",
                help=labels.COLUMN_HELP["has_refs"],
            )

            fig = px.histogram(
                x=comments_df["comments_len"].drop_nulls().clip(upper_bound=300),
                nbins=60,
                title="Comments Length Distribution",
                labels={"x": "Characters", "count": "Papers"},
                height=400,
            )
            fig.update_traces(marker_color="#ab63fa")
            st.plotly_chart(fig, width="stretch")
